...
const ExtAndIntCommon = {
  ...
  '&:hover': { background: 'rgba(0, 0, 255, 0.08)' },
  '&.compactNavItem': {
    overflow: 'hidden',
...
const InternalLink = styled(Box)(({ theme }) => ({
  ...
  '& .navItemActive': {
    ...
    backgroundColor: '#2E23FB !important',
    color: '#fff',
  },
}));
          ...
          >
            <ButtonBase key={item.name} name="child" sx={{ width: '100%' }}>
              {(() => {
                if (item.icon) {
                  return (
                    <Icon className="icon" style={{ fontSize: '24px' }}>
                      {item.icon}
                    </Icon>
                  );
                } else {
        ...
        return (
          <InternalLink key={index}>
            ...
            >
              <ButtonBase key={item.name} name="child" sx={{ width: '100%' }}>
                {item?.icon ? (
                  <Icon className="icon" sx={{ width: 36 }} style={{ fontSize: '24px' }}>
                    {item.icon}
                  </Icon>
                ) : (
                  <Fragment>
